...
<head>
    ...
    <title>Users - <?php echo BLOG_NAME?></title>
    <!-- BEGIN: CSS Assets-->
...
<h4 style="margin:30px;font-size:20px;color:darkred">Users</h4>
...
<tbody>
    <?php
$i = 1;
$run=$db_con->prepare("select * from users");
$run->execute();
while ($row=$run->fetch())
{
$userID = $row['u_id'];
...
>
  <a href="view-user.php?id=<?php echo $userID?>"> <?php echo $i; $i++;?></a>
    ...
    class="px-5 py-3 border-b dark:border-darkmode-300 whitespace-nowrap"
>
   <a href="view-user.php?id=<?php echo $userID?>"> <?php echo $fullname?></a>
</td>
                                            <td
    data-tw-merge
    class="px-5 py-3 border-b dark:border-darkmode-300 whitespace-nowrap"
...
<td><td><a href="delete.php?id=<?php echo $userID?>&type=user">Delete</a></td></td>
</tr>
     <?php } ?>                                 
</tbody>
</table>
